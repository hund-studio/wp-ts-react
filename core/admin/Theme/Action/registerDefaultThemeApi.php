...
new ApiRoute("GET", "/frontpage", function () {
  ...
  if (!$frontpageSlug) {
    return new \WP_Error("frontpage_not_found", "No Static Frontpage set", [
      "status" => 404,
    ]);
  }

  $WP_restResponse = rest_do_request(
    new \WP_REST_Request(
      "GET",
      Helpers::makeFullUrl(
...
new ApiRoute("GET", "/taxonomies", function (\WP_REST_Request $request) {
  ...
  $taxonomies = array_map(function ($instance) {
    ...
    $taxonomy = [
      ...
      "terms" => get_terms($wpObject->name, ["hide_empty" => false]),
    ];
  ...
  if (isset($queryParameters["post_type"])) {
    ...
    $taxonomies = array_filter(
      $taxonomies,
      fn($taxonomy) => !array_diff($queriedPostTypes, $taxonomy["available_in"])
    );
  }
